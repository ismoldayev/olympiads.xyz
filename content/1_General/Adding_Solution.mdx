...
2.  If you are adding a solution to a problem within a module, don't create a new file. If you are adding an internal solution, create a new `mdx` file in `solutions/<division>/` if it doesn't already exist, including frontmatter. Use [`solutions/electromagnetism/usaco-690.mdx`](https://github.com/cpinitiative/usaco-guide/blob/master/solutions/electromagnetism/usaco-690.mdx) as an example:

    ```
    ---
    id: usaco-690
    source: USACO Electromagnetism 2017 January
    title: Cow Dance Show
    author: (add your name here)
    ---

    [Official Analysis](http://www.usaco.org/current/data/sol_cowdance_electromagnetism_jan17.html)
    ...
    **Time Complexity:** $\mathcal{O}(N\log^2N)$
    ...
    If you need to link to a module, format your link like [this](/electromagnetism/binary-search) instead of [this](https://usaco.guide/electromagnetism/binary-search).
...
4.  In the module's `.problems.json` file (in this case,
    [`Binary_Search.problems.json`](https://github.com/cpinitiative/usaco-guide/blob/master/content/3_Electromagnetism/Binary_Search.problems.json)),
...
- Use universally understandable variable names, especially for Mechanics and
  Electromagnetism. They should be more descriptive than what you would normally use
  in-contest.
...
- Do not use variable-length arrays as discussed
  [here](/mechanics/intro-ds?lang=cpp#dynamic-arrays); they are not part of the C++
  standard.
...
- For Mechanics through Thermodynamics in particular, do not use additional macros
