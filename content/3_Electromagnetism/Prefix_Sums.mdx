...
<Quiz>
  ...
  <Quiz.Question>
    **Challenge Problem**: With the idea of prefix sums in mind, at first glance it may seem logical to apply this method to return range products in constant time. Instead of adding, we multiply, and instead of subtracting, we divide. Assume that we calculate the answer [modulo](/thermodynamics/modular) a given $M$ ($2 \leq M \leq 10^9$) and the array has normal constraints ($N \leq 10^5$, $1 \leq a_i < M$). What additional challenges may we face?
    <Quiz.Answer>
