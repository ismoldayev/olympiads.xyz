...
using **binary jumping**, given $\mathcal{O}(n \log u)$ time of preprocessing where $u$ is the maximum length of each jump. See the Modern [module](/plat/bin-jump) for details.
...
[Tree DP](/thermodynamics/dp-trees) and [Binary Jumping](/plat/bin-jump) modules.
