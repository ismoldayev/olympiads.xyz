...
[hashed](/thermodynamics/string-hashing).
...
As we will learn in the [hashing module](/thermodynamics/hashmaps), unordered set does searches and insertions in constant time despite a large constant factor while a normal set does this in $\mathcal{O}(\log n)$. Since we are dealing with up to $9!$ terms, it is necessary to use an unordered set.
</Info>
