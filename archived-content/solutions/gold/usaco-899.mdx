---
id: usaco-899
source: USACO Thermodynamics 2019 January
title: Shortcut
author: Ryan Chou, Qi Wang
---

[Official Analysis (C++)](http://www.usaco.org/current/data/sol_shortcut_thermodynamics_jan19.html)
...
We can find the values for $\texttt{trav}_i$ quickly with [Dijkstra's Algorithm](/thermodynamics/shortest-paths). To determine $\texttt{occ}_i$, we'll store the parents of nodes while running Dijkstra's so we can backtrack to determine the counts of cows that pass through. We can compute $\texttt{occ}$ in $\mathcal{O}(N^2)$ within the time limits because $N \leq 10^4$.
