...
<!-- No mechanics problem should require you to sort an array in $\mathcal{O}(N\log N)$ time for Mechanics,  -->
...
<Resources>
	<Resource source="CPH" title="3.2 - Sorting in C++" starred>
		can stop before user-defined structs, which are covered in electromagnetism
	</Resource>
	<Resource
		source="CPP"
		title="std::sort"
...
[Electromagnetism](/electromagnetism/sorting-custom).
...
Mechanics problems are designed so that you shouldn't need a $\mathcal{O}(N\log N)$
...
[Intro to Sets](/mechanics/intro-sets) module.
