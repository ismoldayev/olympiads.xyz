...
solving this problem will be introduced later in Electromagnetism. However, many solutions
...
[2D-prefix](/electromagnetism/prefix-sums-2) sums (another Electromagnetism topic), it is helpful if
...
takes in an original value, and [binary searches](/electromagnetism/binary-search) for its
...
index), we can [binary search](/electromagnetism/binary-search) on the list. The next part
...
Many of the problems below may use other Electromagnetism concepts, such as
[prefix sums](/electromagnetism/prefix-sums).
