...
const ProgressDropdown = ({ onProgressSelected, currentProgress }) => {
  ...
  const icon = (status: ProblemProgress, equal: boolean) => {
    const colorMap: { [key in ProblemProgress]: string } = {
      Незапочната: '',
      Решавам: 'text-orange-500',
      Решена: 'text-green-400',
      Преглеждам: 'text-red-500',
      Прескочена: 'text-blue-400',
      Игнорирана: 'text-purple-400',
    };
...
export default function ProblemStatusCheckbox({
  ...
  const updateModuleProgressToPracticing = () => {
    ...
    const moduleProgress =
      ...
      'Незапочнат';
    if (moduleProgress !== 'Незапочнат') return;
    setModuleProgress(markdownLayoutInfo.id, 'Practicing');
  };
  const status: ProblemProgress =
    userProgressOnProblems[problem.uniqueId] || 'Незапочната';
  const color: { [key in ProblemProgress]: string } = {
    Незапочната: 'bg-gray-200 dark:bg-gray-700',
    Решавам: 'bg-orange-400 dark:bg-orange-400',
    Решена: 'bg-green-500 dark:bg-green-600',
    Преглеждам: 'bg-red-500 dark:bg-red-600',
    Игнорирана: 'bg-red-100 dark:bg-red-900',
    Прескочена: 'bg-blue-300 dark:bg-blue-700',
  };
        ...
        <div className="w-56 z-20">
          <ProgressDropdown
            onProgressSelected={progress => {
              ...
              const solved = x => x == 'Преглеждам' || x == 'Solved';
              if (progress == 'Решавам' || solved(progress)) {
